...
HARD_CONSTRAINTS = (
    ...
    "4) 主视觉必须从本页标题、副标题、要点里的具体动作、物件、关系或场景出发，不能跳到和文本只弱相关的抽象隐喻；"
    "5) 可以适度视觉化，但抽象度要低：用户不看正文也能猜到这页在讲什么；"
    "6) 画面要丰富、有层次、有材质光影和景深，但保持秩序和一个明确焦点。"
)
...
def _build_messages(
    ...
    system = (
        ...
        "你的创意原则是贴近文本，不炫技，不把普通观点翻译成过度抽象的艺术隐喻。"
    )
    user = (
        f"系列选题：{topic}\n"
        ...
        f"请为每一页创意一个贴近文案的画面 brief。严格只输出一个 JSON 数组，共 {len(cards)} 个元素，"
        "每个元素形如 "
        '{"index": 1, "metaphor": "本页主画面（必须能直接对应标题/要点，不要过度抽象）", '
        '"composition": "构图与焦点位置，说明它对应了哪句文案", "details": "从文案提取的支撑元素、材质、光影、景深等细节", '
        '"accent_focus": "强调色用在哪里"}。'
        "如果文本里有具体对象、动作、流程、边界、记录、工具、路径，优先画这些；"
        "只有文本本身很抽象时，才使用低抽象度隐喻。"
        "不要输出 JSON 以外的任何内容。"
    )
